...
async function uploadToIPFS() {
    ...
    const idx = document.getElementById('token-select').value;
    if (!key || !idx) { alert('Enter Pinata API key and select token'); return; }
    ...
    try {
        ...
        const metadata = {
            ...
            description: token.description || '', image: token.image || '',
            external_url: token.external_url || '', attributes: token.attributes || []
        };
        const response = await fetch('https://api.pinata.cloud/pinning/pinJSONToIPFS', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + key },
            body: JSON.stringify({ pinataContent: metadata, pinataMetadata: { name: token.name + '-metadata' } })
        });
