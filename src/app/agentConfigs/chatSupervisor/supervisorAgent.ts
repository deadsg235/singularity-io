...
let supervisorPromptCache: string | null = null;

async function getSupervisorAgentInstructions(): Promise<string> {
  if (supervisorPromptCache) {
    return supervisorPromptCache;
  }
  const prompt = await fetchPromptModule('agent.supervisor.instructions');
  supervisorPromptCache = prompt.segment;
  return supervisorPromptCache;
}
      ...
      input: [
        {
          type: 'message',
          role: 'system',
          content: await getSupervisorAgentInstructions(),
        },
        {
          type: 'message',
          role: 'user',
