...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(getDexterApiRoute("/prompt-profiles/preview"), {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          ok: false,
          error: "prompt_profile_preview_failed",
    ...
    console.error("/api/prompt-profiles/preview POST error", error);
